...
?>
<aside id="sidebar" class="bg-slate-900 text-white w-64 flex-shrink-0 flex-col fixed inset-y-0 left-0 md:relative md:flex">
    ...
    <nav class="flex-1 mt-6 space-y-2 px-2">
        <?php
        $activeClass = "bg-slate-700/50 text-white";
        $inactiveClass = "text-slate-300 hover:bg-slate-700 hover:text-white";

        // --- MENU YANG BISA DILIAT SEMUA ROLE (Contoh: Dashboard) ---
        ?>
        <!-- <a href="<?= BASE_URL ?>../views/purchasing_dashboard.php" class="nav-link flex items-center py-3 px-4 transition duration-200 rounded-lg <?= ($currentPage === 'dashboard') ? $activeClass : $inactiveClass ?>">
        ...
        if ($userRole === 'Admin'):
        ?>
            <a href="<?= BASE_URL ?>model/manage_user.php" class="nav-link flex items-center py-3 px-4 transition duration-200 rounded-lg <?= ($currentPage === 'manage_user') ? $activeClass : $inactiveClass ?>">
                <i class="fas fa-users-cog w-6 text-center text-lg"></i><span class="sidebar-text ml-4">Kelola Pengguna</span>
            </a>
            <a href="#" class="nav-link flex items-center py-3 px-4 transition duration-200 rounded-lg">
                <i class="fas fa-chart-line w-6 text-center text-lg"></i><span class="sidebar-text ml-4">Laporan Penjualan</span>
            </a>
        <?php
        endif;
        ?>
        ...
        <?php
        // --- MENU YANG BISA DILIAT LEBIH DARI SATU ROLE (Contoh: Admin & Purchasing) ---
        if (in_array($userRole, ['Admin', 'Purchasing'])):
        ?>
            <a href="#" class="nav-link flex items-center py-3 px-4 transition duration-200 rounded-lg <?= ($currentPage === 'items') ? $activeClass : $inactiveClass ?>">
                <i class="fas fa-box-open w-6 text-center text-lg"></i><span class="sidebar-text ml-4">Daftar Barang</span>
            </a>
        <?php
        endif;
        ?>
